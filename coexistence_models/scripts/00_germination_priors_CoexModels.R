...
## - zero-germination rows are retained
...
############################
required_packages <- c("here", "readr", "dplyr", "tidyr", "stringr", "tibble")
...
data_file <- here::here("data", "field", "collated_data_18_02_2020.csv")

stopifnot(file.exists(data_file))

base_dir <- here::here("coexistence_models", "00_germination_priors")
...
dat <- readr::read_csv(
  data_file,
  na = c("", "NA", "NaN", "NULL", "#N/A", "#VALUE!", "n/a"),
...
trach <- dat %>%
  dplyr::mutate(
    ...
    researcher     = str_trim(str_to_lower(researcher))
  ) %>%
  filter(
    genus == "trachymene",
...
trach_direct <- trach %>%
  filter(
    ...
    is.na(researcher) | researcher != "hao ran"
  ) %>%
  dplyr::mutate(
...
germination_priors <- list(
  input_file = normalizePath(data_file),
...
note_lines <- c(
  ...
  "- Zero-germination rows are retained because they are valid binomial observations unless known to be failed trials.",
  "- Main germination fraction is field_number_germinants / field_number_seeds.",
  "- Nonviable seeds are retained in the denominator, so this estimates realised germination from total field seeds, not germination conditional on viability.",
  "- Lost seeds are retained in the denominator in the main analysis; this is conservative and should ideally be checked with a sensitivity analysis.",
...
stopifnot(file.exists(germ_prior_file))

germination_priors <- readRDS(germ_prior_file)

germ_site_priors <- germination_priors$site_priors %>%
  dplyr::mutate(
...
stopifnot(
  all(c("Site", "focsp", "g_source", "g_alpha_final", "g_beta_final", "g_mean_final") %in%
        names(germ_site_priors))
)
